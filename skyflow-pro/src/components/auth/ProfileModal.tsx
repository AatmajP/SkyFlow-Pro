...
export function ProfileModal({ isOpen, onClose }: ProfileModalProps) {
    ...
    const { bookings } = useBookingStore() // Get real bookings
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setEmail('')
            setPassword('')
            onClose() // Auto-close on successful login for better UX
        } catch (error) {
    ...
    return (
        <>
            ...
            {/* Modal */}
            <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl z-50 animate-scale-in">
                {/* Header */}
