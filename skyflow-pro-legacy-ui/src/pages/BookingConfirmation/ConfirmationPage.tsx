...
import { downloadETicket, printItinerary, shareBooking, type ConfirmationData } from '@/utils'

// Use the formatter for currency
...
export function ConfirmationPage() {
    ...
    const handleShare = async () => {
        if (!booking) return
        const success = await shareBooking(booking)
        if (success) {
            // In a real app, use a toast here. For now, we rely on system share UI or implicit clipboard action
            // Or we could show a temporary "Copied!" state on the button
        }
    }
        ...
        return (
            <div className="min-h-screen flex items-center justify-center">
                <div className="glass rounded-2xl p-8 text-center max-w-md">
                    <h2 className="text-xl font-semibold text-slate-50">Booking not found</h2>
                    <p className="text-sm text-slate-400 mt-2">The booking confirmation could not be found.</p>
    ...
    return (
        <div className="min-h-screen relative overflow-hidden">
            {/* Confetti animation (CSS-based) - Hidden in Print */}
            {showConfetti && (
            ...
            {/* Background decorations - Hidden in Print implicitly via background reset, but adding no-print saves ink */}
            <div className="fixed top-20 right-20 h-96 w-96 rounded-full bg-emerald-500/10 blur-3xl pointer-events-none animate-pulse no-print" />
            <div className="fixed bottom-20 left-20 h-72 w-72 rounded-full bg-sky-500/10 blur-3xl pointer-events-none animate-pulse no-print" />
            ...
            <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                <div id="printable-content">
                    ...
                    <div className="text-center mb-10 animate-scale-in">
                        <div className="inline-flex items-center justify-center h-24 w-24 rounded-full bg-emerald-500/20 mb-6 animate-pulse-glow no-print">
                        ...
                        <div className="mt-6 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800/50 border border-slate-700">
                            <span className="text-sm text-slate-400">Confirmation Number:</span>
                    ...
                    <div className="glass rounded-3xl p-6 sm:p-8 mb-8 animate-fade-in">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-4 pb-6 border-b border-slate-800/50">
                            <div className="flex items-center gap-4">
                                <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-sky-500 to-purple-600 flex items-center justify-center no-print">
                        ...
                        <div className="mt-4 p-5 rounded-2xl bg-gradient-to-r from-slate-800/50 to-slate-900/50 border border-dashed border-slate-700">
                            <div className="flex flex-wrap items-center justify-between gap-4">
                                <div>
                                    <p className="text-xs text-slate-500 uppercase tracking-wider">Boarding Pass</p>
                        ...
                        <div className="mt-6 p-4 rounded-xl bg-sky-950/30 border border-sky-500/20">
                            <p className="text-sm font-medium text-sky-300">Baggage Allowance</p>
                    ...
                    <div className="glass rounded-2xl p-5 mb-8 animate-fade-in" style={{ animationDelay: '0.2s' }}>
                        <div className="flex items-center gap-4">
                            <div className="h-12 w-12 rounded-xl bg-emerald-500/20 flex items-center justify-center shrink-0 no-print">
                                <Mail className="h-6 w-6 text-emerald-400" />
                            </div>
                            <div>
                                <p className="font-medium text-slate-200">Confirmation email sent</p>
                                <p className="text-sm text-slate-400">
                                    We've sent your booking confirmation and e-ticket to <span className="text-sky-400">{passenger.email}</span>
                                    <span className="block text-xs text-slate-500 italic mt-1 no-print">(Note: This is a demo, no real email was sent)</span>
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex flex-wrap items-center justify-center gap-4 animate-fade-in no-print" style={{ animationDelay: '0.3s' }}>
                    <button
                        onClick={() => downloadETicket(booking)}
                        className="btn-secondary flex items-center gap-2 hover:bg-slate-800 hover:text-sky-400 transition-colors"
                    >
                ...
                <div className="text-center mt-12 animate-fade-in no-print" style={{ animationDelay: '0.4s' }}>
                    <p className="text-slate-400 mb-4">Need to book another flight?</p>
                ...
                {/* Footer */}
                <footer className="mt-12 pt-6 border-t border-slate-800/50 text-center no-print">
